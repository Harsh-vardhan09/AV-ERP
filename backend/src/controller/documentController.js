...
const renderCertificatePdfToFile = async ({ snapshot, outputPath }) => {
  ...
  if (templateSnapshot.templateImageUrl) {
    try {
      ...
      console.warn('renderCertificatePdfToFile: could not fetch template image', e.message);
    }
  }
...
exports.lockDocument = async (req, res) => {
  try {
    ...
    console.error('lockDocument', err);
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.unlockDocument = async (req, res) => {
  try {
    ...
    console.error('unlockDocument', err);
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.uploadTemplateImage = async (req, res) => {
  try {
    ...
    console.error('uploadTemplateImage', err);
    return res.status(500).json({ success: false, message: err.message });
  }
};
  ...
  try {
    ...
    if (template.layoutMode === 'structured') {
      ...
      try {
        ...
        console.error('generateFromTemplate:puppeteer', pdfErr.message);
      ...
      try {
        ...
        console.error('generateFromTemplate:pdfkit', pdfErr.message);
      }
    }
    ...
    console.error('generateFromTemplate', err);
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.downloadCertificatePdf = async (req, res) => {
  try {
    ...
    console.warn(`downloadCertificatePdf: file missing for doc ${doc._id} — regenerating on demand`);
    ...
    console.error('downloadCertificatePdf', err);
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.saveTemplateLayout = async (req, res) => {
  try {
    ...
    console.error('saveTemplateLayout', err);
    return res.status(500).json({ success: false, message: err.message });
  }
};
