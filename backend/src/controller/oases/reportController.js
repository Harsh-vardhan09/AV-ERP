// ══════════════════════════════════════════════════════════════════
// OASES Controller — Report (Sprint 6)
...
exports.generateResults = oasesAsync(async (req, res) => {
  ...
  if (results.length === 0) {
    return oasesError(res, 'No result sheets found. Lock sheets first.', 400);
  }
...
exports.publishResults = oasesAsync(async (req, res) => {
  ...
  if (results.length === 0) {
    return oasesError(res, 'No unpublished results found.', 400);
  }
  ...
  let Student;
  try { Student = require('../../models/Student'); } catch { Student = null; }
...
exports.listResults = oasesAsync(async (req, res) => {
  ...
  return oasesSuccess(res, { results, total, page: Number(page), hasMore, nextPage }, 'Results fetched.');
});
...
exports.getStudentResult = oasesAsync(async (req, res) => {
  ...
  if (!match) return oasesError(res, 'Result not found or not yet published.', 404);

  return oasesSuccess(res, match, 'Student result fetched.');
});
...
exports.getEvaluatorStats = oasesAsync(async (req, res) => {
  ...
  return oasesSuccess(res, { evaluators: Object.values(evalMap) }, 'Evaluator stats fetched.');
});
