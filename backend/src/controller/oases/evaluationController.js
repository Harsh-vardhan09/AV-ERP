// ══════════════════════════════════════════════════════════════════
// OASES Controller — Evaluation (Sprint 3 — THE CORE)
...
const { SHEET_STATUS, EVAL_ROUNDS } = require('../../utils/oasesConstants');
...
const { validateMarks, calculateTotals } = require('../../services/oases/marksValidation.service');
const { checkConflict } = require('../../services/oases/conflict.service');
const { lockSheet }     = require('../../services/oases/result.service');
const { safeRedisOperation } = require('../../config/oasesRedis');
const { validateMCQMark } = require('../../services/oases/mcq.service');
...
exports.getSheetForEval = oasesAsync(async (req, res) => {
  ...
  if (
    ...
    console.log(`[evalController] Auto-healing unprocessed sheet ${sheet._id}...`);
    try {
      ...
      console.log(`[evalController] Auto-heal complete: ${sheet.totalPages} pages.`);
    } catch (healErr) {
      console.error('[evalController] Auto-heal failed:', healErr.message);
    }
  }
...
exports.getPageUrl = oasesAsync(async (req, res) => {
  ...
  if (pageIdx < 0 || pageIdx >= actualTotalPages) {
    return oasesError(res, `Page ${pageNo} does not exist.`, 400);
  }
...
exports.saveMark = oasesAsync(async (req, res) => {
  ...
  if (questionNo === undefined) return oasesError(res, 'questionNo is required.', 400);
  ...
  if (!sheet) return oasesError(res, 'Sheet not found or not assigned to you.', 404);
  if (sheet.status === SHEET_STATUS.LOCKED) return oasesError(res, 'Sheet is locked.', 403);
  ...
  if (question && !isNA && marksGiven > question.maxMarks) {
    return oasesError(res, `Marks (${marksGiven}) exceed max (${question.maxMarks}) for Q${questionNo}.`, 400);
  }

  // Sprint 5: MCQ validation — server recomputes score from answer key
  if (question?.questionType === 'mcq' && !isNA) {
    ...
    if (studentOption) {
      ...
      if (!valid) {
        return oasesError(res, `MCQ mark tampered: expected ${expected} for Q${questionNo}, got ${marksGiven}.`, 400);
      }
    }
  }
...
exports.saveDraft = oasesAsync(async (req, res) => {
  ...
  if (!sheet) return oasesError(res, 'Sheet not found or not assigned.', 404);
  ...
  return oasesSuccess(res, { savedAt: evalMark.savedAt }, 'Draft saved.');
});
...
exports.getDraft = oasesAsync(async (req, res) => {
  ...
  return oasesSuccess(res, draft || null, 'Draft fetched.');
});
...
exports.markPageReviewed = oasesAsync(async (req, res) => {
  ...
  if (isAdminRole) {
    return oasesSuccess(res, { pageNo: Number(pageNo) }, 'Page reviewed (admin view).');
  }
  ...
  return oasesSuccess(res, { pageNo: Number(pageNo) }, 'Page marked as reviewed.');
});
...
// ══════════════════════════════════════════════════════════════════
// POST /evaluation/submit/:sheetId — Submit final marks (Sprint 4)
// ══════════════════════════════════════════════════════════════════
exports.submitMarks = oasesAsync(async (req, res) => {
  ...
  // ── Sprint 4: Server-side validation ──────────────────────────
  ...
  if (!validation.isValid) {
    ...
    console.error('[submitMarks] Validation failed for sheet', sheetId, '\nErrors:', JSON.stringify(validation.errors, null, 2));
    return oasesError(res, 'Validation failed. Please fix errors before submitting.', 400, validation.errors);
  }
  ...
  // ── Sprint 4: Redis daily count incr ──────────────────────────
  let dailyCount = 1;
  ...
  // ── Sprint 4: Post-submit pipeline (fire-and-forget) ──────────
  setImmediate(async () => {
    try {
      ...
      console.error('[evaluationController] post-submit pipeline error:', err.message);
    }
  });
...
exports.approveSheet = oasesAsync(async (req, res) => {
  ...
  const userRole    = req.user?.oasesRole;
  const isAdminRole = ['SCHOOL_ADMIN', 'SUPER_ADMIN'].includes(userRole);
  if (!isAdminRole) return oasesError(res, 'Only admin can approve sheets.', 403);
  ...
  return oasesSuccess(res, { sheetId: sheet._id, status: SHEET_STATUS.APPROVED }, 'Sheet approved.');
});
...
exports.overrideMarks = oasesAsync(async (req, res) => {
  ...
  const userRole    = req.user?.oasesRole;
  const isAdminRole = ['SCHOOL_ADMIN', 'SUPER_ADMIN'].includes(userRole);
  if (!isAdminRole) return oasesError(res, 'Only admin can override marks.', 403);
