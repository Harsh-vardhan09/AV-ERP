// ══════════════════════════════════════════════════════════════════
// OASES Service — Result Lock (Sprint 4)
...
const lockSheet = async (sheetId, schoolId, finalMarks, opts = {}) => {
  ...
  if (!examConfig) {
    examConfig = await Exam.findById(sheet.examConfigId)
      .select('totalMarks passingMarks conflictThreshold doubleEval name').lean();
  }
  ...
  const headMark = await EvaluationMark.findOne({ sheetId, round: EVAL_ROUNDS.HEAD,    isDraft: false }).lean();
  const r1Mark   = await EvaluationMark.findOne({ sheetId, round: EVAL_ROUNDS.ROUND_1, isDraft: false }).lean();
  const r2Mark   = await EvaluationMark.findOne({ sheetId, round: EVAL_ROUNDS.ROUND_2, isDraft: false }).lean();
  const accepted = headMark || r1Mark;

  const percentage = examConfig.totalMarks > 0
  ...
  emitToAll('oases:sheet:locked', {
    sheetId, examConfigId: sheet.examConfigId,
    finalMarks, percentage, isPassed,
  });
