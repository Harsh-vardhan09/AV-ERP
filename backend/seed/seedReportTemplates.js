/**
 * seedReportTemplates.js — register the bundled report card templates.
 *
 * Reads the HTML from src/templates/ and upserts one ReportTemplate document
 * per template as a GLOBAL template. Idempotent: re-running updates htmlContent
 * and templateSchema in place rather than creating duplicates, so editing a
 * template file and re-seeding pushes the change through.
 *
 * Run standalone:   node seed/_runner.js seedReportTemplates.js
 * Or via the runner: npm run seed:all
 *
 * Templates are platform-wide (isGlobal:true, schoolId:null) — Super Admins
 * author them, schools adopt one via SchoolSettings.selectedReportTemplateId.
 */
...
const SCHEMAS      = require('../src/templates/schemas');
