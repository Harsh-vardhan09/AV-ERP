 ...
 */
export const validateAllMarks = (marksMap, sections, examConfig, pagesReviewed = [], totalPages = 0) => {
  const errors   = [];
  const warnings = [];
  ...
  Object.entries(optGroups).forEach(([groupId, { allowed, attempted }]) => {
    if (attempted > allowed) {
      errors.push({ questionNo: null, msg: `Group ${groupId}: ${attempted} answered but only ${allowed} allowed.` });
    } else if (attempted < allowed) {
      warnings.push({ questionNo: null, msg: `Group ${groupId}: Only ${attempted}/${allowed} attempted.` });
    }
  });
  ...
  if (examConfig?.totalMarks && grandTotal > 0.9 * examConfig.totalMarks) {
    warnings.push({ questionNo: null, msg: `Total (${grandTotal}) is unusually high (> 90% of ${examConfig.totalMarks}).` });
  }
