...
export const reportService = {
  ...
  generate: (examId) =>
    oasesAxios.post(`/report/generate/${examId}`).then((r) => r.data.data),

  /** Publish: decrypt rollNo, link student, set isPublished */
  publish: (examId) =>
    oasesAxios.post(`/report/publish/${examId}`).then((r) => r.data.data),

  /** Summary stats + all chart datasets */
  getExamSummary: (examId) =>
    oasesAxios.get(`/report/exam/${examId}`).then((r) => r.data.data),

  /** Paginated results table — supports useInfiniteQuery */
  listResults: (examId, page = 1, search = '') =>
    oasesAxios.get(`/report/results/${examId}`, { params: { page, limit: 50, search } })
      .then((r) => r.data.data),
  ...
  downloadPDF: (sheetId) =>
    oasesAxios.get(`/report/sheet/${sheetId}/pdf`, { responseType: 'blob' })
      .then((r) => r.data),
};
