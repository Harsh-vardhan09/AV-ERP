...
const _BASE_URL = import.meta.env.VITE_API_BASE_URL || import.meta.env.VITE_PORT || 'http://localhost:5000';
...
export function useServerHealth() {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      const interval = (status === 'offline' || status === 'degraded')
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(function tick() {
      checkHealth().then(() => {
        if (isMounted.current) {
          const next = (status === 'offline' || status === 'degraded')
            ? DEGRADED_CHECK_INTERVAL
            : INITIAL_CHECK_INTERVAL;
