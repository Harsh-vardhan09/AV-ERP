...
import { useApplyLeaveMutation, useGetMyLeavesQuery } from '@modules/people/api/teacherApi';
...
const TeacherLeave = () => {
  ...
  const { data: leaveData, isLoading } = useGetMyLeavesQuery();
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [activeTab, setActiveTab] = useState('all');
  const [form, setForm] = useState({ leaveType: 'casual', startDate: '', endDate: '', reason: '' });

  const leaves = useMemo(() => leaveData?.data || [], [leaveData]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4 px-2 sm:px-4 pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-1">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-slate-900">Faculty Leave Portal</h1>
