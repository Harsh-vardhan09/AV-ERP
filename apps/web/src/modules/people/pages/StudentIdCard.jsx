...
function StudentCard({ student, schoolName, schoolAddress, schoolPhone, schoolLogo, session }) {
  ...
  return (
    <div style={{
      width: 320, height: 202, border: '2px solid #1e3a5f', borderRadius: 10,
      ...
      <div style={{
        ...
        {schoolLogo
          ...
          : <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, color: '#1e3a5f' }}>S</div>
        }
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 8, opacity: 0.85, letterSpacing: 0.4 }}>STUDENT IDENTITY CARD</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, padding: '7px 10px', display: 'flex', gap: 9 }}>
        ...
        <div style={{ flexShrink: 0 }}>
          {photo
            ? <img src={photo} alt={fullName} style={{ width: 58, height: 72, objectFit: 'cover', border: '2px solid #1e3a5f', borderRadius: 5 }} />
            : <div style={{ width: 58, height: 72, border: '2px solid #1e3a5f', borderRadius: 5, background: 'linear-gradient(135deg,#dbeafe,#eff6ff)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, color: '#1e3a5f' }}>
                {`${student.firstName?.[0] || ''}${student.lastName?.[0] || ''}`.toUpperCase() || 'S'}
              </div>
          }
          <div style={{ marginTop: 3, textAlign: 'center', background: '#1e3a5f', color: '#fff', borderRadius: 3, padding: '1px 4px', fontSize: 8.5, fontWeight: 700 }}>{blood}</div>
        </div>

        {/* Info */}
        <div style={{ flex: 1, minWidth: 0, fontSize: 9, lineHeight: 1.45 }}>
          <div style={{ fontWeight: 700, fontSize: 11.5, color: '#1e3a5f', marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{fullName}</div>
          {[
            ...
            <div key={label} style={{ display: 'flex', gap: 4 }}>
              <span style={{ color: '#4b5563', fontWeight: 600, minWidth: 58, flexShrink: 0 }}>{label}:</span>
      ...
      {/* Footer */}
      <div style={{ background: '#f1f5f9', borderTop: '1px solid #e2e8f0', padding: '3px 10px', display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 8, color: '#475569', flexShrink: 0, whiteSpace: 'nowrap', overflow: 'hidden' }}>
