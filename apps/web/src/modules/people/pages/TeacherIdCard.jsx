...
import { useGetAllTeachersEnhancedQuery } from '@modules/people/api/teacherManagementApi';
...
function TeacherCard({ teacher, schoolName, schoolAddress, schoolPhone, schoolLogo, session }) {
  const fullName   = `${teacher.firstName || ''} ${teacher.lastName || ''}`.trim() || '—';
  const subject    = teacher.subjectSpecialization || teacher.subject || '—';
  const empId      = teacher.employeeId || teacher.teacherId || '—';
  const qual       = teacher.qualification || '—';
  const phone      = teacher.phone || teacher.contactNumber || '—';
  const blood      = teacher.bloodGroup || '—';
  const photo      = teacher.photo || teacher.documents?.photo;
...
export default function TeacherIdCard() {
  ...
  const { data, isLoading, isFetching } = useGetAllTeachersEnhancedQuery({ page, limit, search });
    ...
    return (
      <>
        ...
        <div className="no-print" style={{ padding: '16px 24px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => setPrintTeacher(null)} style={{ padding: '8px 18px', borderRadius: 8, border: '1px solid #d1d5db', background: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>← Back</button>
          <button onClick={() => window.print()} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#16a34a', color: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 13 }}>🖨️ Print</button>
          <span style={{ color: '#64748b', fontSize: 13 }}>ID Card Preview — {printTeacher.firstName} {printTeacher.lastName}</span>
        </div>
        <div style={{ padding: 32, display: 'flex', justifyContent: 'center', background: '#f8fafc', minHeight: '80vh' }}>
          <TeacherCard teacher={printTeacher} schoolName={schoolName} schoolAddress={schoolAddress} schoolPhone={schoolPhone} schoolLogo={schoolLogo} session={session} />
        </div>
      </>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Teacher ID Cards</h1>
          <p className="text-xs text-gray-500 mt-0.5">Total: <strong>{pagination.total ?? (Array.isArray(teachers) ? teachers.length : 0)}</strong> teachers</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg px-4 py-3 mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-gray-600">
          <span>Show</span>
          <select value={limit} onChange={(e) => { setLimit(Number(e.target.value)); setPage(1); }} className="border border-gray-300 rounded-md px-2 py-1 text-sm outline-none focus:ring-2 focus:ring-green-500">
            {[10, 20, 50, 100].map(n => <option key={n} value={n}>{n}</option>)}
          </select>
          <span>entries</span>
        </div>
        <div className="relative">
          <input value={searchInput} onChange={(e) => handleSearch(e.target.value)} placeholder="Search teacher by name…" className="border border-gray-300 rounded-md pl-8 pr-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-green-500 w-64" />
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        {(isLoading || isFetching) ? (
          ...
          <div className="text-center py-16 text-gray-400">
            <svg className="w-12 h-12 mx-auto mb-3 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  ...
                  <th className={th} style={{ minWidth: 200 }}>Teacher Name</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {teachers.map((t, idx) => {
                  ...
                  const photo    = t.photo || t.documents?.photo;
                  ...
                  return (
                    <tr key={t._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className={td}>
                        {photo
                          ? <img src={photo} alt={fullName} className="w-9 h-9 rounded-full object-cover border border-gray-200" />
                          : <div className="w-9 h-9 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 text-white flex items-center justify-center text-xs font-bold">{`${t.firstName?.[0] || ''}${t.lastName?.[0] || ''}`.toUpperCase()}</div>
                        }
                      </td>
                      <td className={td}><span className="font-medium text-gray-800">{fullName || '—'}</span></td>
                      <td className={td}><span className="font-medium text-gray-700">{t.employeeId || t.teacherId || '—'}</span></td>
                      <td className={td}><span className="px-2 py-0.5 bg-green-50 text-green-700 border border-green-200 rounded text-xs font-medium">{t.designation || 'Teacher'}</span></td>
                      <td className={`${td} text-gray-600`}>{t.subjectSpecialization || t.subject || '—'}</td>
                      <td className={`${td} text-gray-600`}>{t.phone || t.contactNumber || '—'}</td>
                      <td className={td}>
                        <button onClick={() => handlePrint(t)} title="Print ID Card"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: Math.min(5, pagination.totalPages) }, (_, i) => {
                ...
                return <button key={p} onClick={() => setPage(p)} className={`px-3 py-1 text-xs border rounded ${p === page ? 'bg-green-600 text-white border-green-600' : 'border-gray-300 hover:bg-gray-50'}`}>{p}</button>;
              })}
