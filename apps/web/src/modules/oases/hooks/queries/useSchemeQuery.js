...
export const usePatchScheme = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ examId, questions }) => examConfigService.patchScheme(examId, questions),
    onSuccess: (_, { examId }) => {
      qc.invalidateQueries({ queryKey: oasesKeys.scheme(examId) });
    },
  });
...
export const useUploadAnswerKey = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ examId, answerKey }) => examConfigService.uploadAnswerKey(examId, answerKey),
    onSuccess: (_, { examId }) => {
      qc.invalidateQueries({ queryKey: oasesKeys.scheme(examId) });
