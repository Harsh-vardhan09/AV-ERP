...
export const examConfigService = {
  ...
  /** Single config by ID */
  get: (id) => oasesAxios.get(`/exam-config/${id}`).then((r) => r.data.data),

  /** Create */
  create: (payload) => oasesAxios.post('/exam-config', payload).then((r) => r.data.data),

  /** Update (blocked server-side if not draft) */
  update: (id, payload) => oasesAxios.patch(`/exam-config/${id}`, payload).then((r) => r.data.data),
  ...
  remove: (id) => oasesAxios.delete(`/exam-config/${id}`).then((r) => r.data),

  /** Get question scheme for an exam config */
  getScheme: (examConfigId) => oasesAxios.get(`/scheme/${examConfigId}`).then((r) => r.data.data),
