...
export const sheetService = {
  ...
  upload: (examConfigId, formData) =>
    oasesAxios
      .post(`/upload/${examConfigId}`, formData, {
...
export const evalService = {
  ...
  getDraft: (sheetId) => oasesAxios.get(`/evaluation/draft/${sheetId}`).then((r) => r.data.data),

  /** Save single question mark (optimistic) */
  saveMark: (payload) => oasesAxios.post('/evaluation/mark', payload).then((r) => r.data.data),
  ...
  getSheet: (sheetId) => oasesAxios.get(`/evaluation/sheet/${sheetId}`).then((r) => r.data.data),
};
