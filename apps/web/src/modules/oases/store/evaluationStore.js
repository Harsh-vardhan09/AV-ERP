...
const useEvaluationStore = create((set, get) => ({
  ...
  toggleAnnotationMode: () => set((state) => ({ annotationMode: !state.annotationMode })),

  setAnnotationTool: (tool) => set({ annotationTool: tool }),

  setZoom: (level) => set({ zoomLevel: Math.min(3.0, Math.max(0.25, level)) }),
  ...
  setSplitRatio: (ratio) => set({ splitRatio: Math.min(80, Math.max(40, ratio)) }),
