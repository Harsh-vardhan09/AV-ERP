...
// Both /login and /staff-login render this. They differ only in which roles the
// dropdown offers and what sits under the button, so the markup lives once.
const LoginForm = ({ roles, defaultRole, emailPlaceholder, footer }) => {
    ...
    <div className="relative flex justify-center items-center min-h-[100dvh] bg-slate-50 py-6 sm:py-10 px-3 sm:px-4 overflow-x-hidden font-sans">
      ...
      <div className="bg-white/75 backdrop-blur-[25px] border border-white/60 shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-6 sm:p-9 rounded-2xl w-full max-w-[420px] relative z-10 my-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-5">
          ...
          {/* ── Login as ──────────────────────────────────────────────── */}
          {roles.length === 1 ? (
            <p className="text-center text-lg sm:text-xl font-semibold text-slate-700 py-1">
              Login as <span className="font-bold text-indigo-600">{roles[0].label}</span>
            </p>
          ) : (
          <div>
