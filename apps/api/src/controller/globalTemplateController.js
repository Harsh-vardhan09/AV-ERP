/**
 * Global Report Template Controller — SUPER ADMIN ONLY.
 *
 * Authors the report card templates shared by every school. Mounted behind
 * verifySuperAdmin (see routes/superAdminRoutes.js), so req.superAdmin is set
 * and req.user / req.schoolId are deliberately absent.
 *
 * Templates written here are { isGlobal: true, schoolId: null }. School admins
 * can read them and adopt one via SchoolSettings.selectedReportTemplateId, but
 * cannot modify them.
 *
 * FORMAT: a template is ONE HTML document with inline CSS (a <style> block is
 * fine). There is no separate cssContent any more — the field is retained on
 * the model only for pre-split rows and is never written here.
 */
...
/**
 * Extract tokens from raw HTML — powers the editor's Fields tab.
 * POST /api/super-admin/templates/extract-fields   body: { htmlContent }
 */
