...
exports.registerStudent = async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error('Student profile creation failed, user rolled back:', profileErr.message);
    ...
    setImmediate(async () => {
      try {
        ...
        logger.debug(`[FeeAutoAssign] ✓ Fee assigned for student ${studentProfile._id}`);
      } catch (feeErr) {
        // If no fee structure exists for this class yet, that's okay — can be done later
        logger.debug(`[FeeAutoAssign] ⚠ Skipped for ${studentProfile._id}: ${feeErr.message}`);
      }
    });
    res.status(201).json({
      success: true,
    ...
    return serviceError(res, 'Student registration error:', error);
  }
};
...
exports.registerTeacher = async (req, res) => {
  try {
    ...
    try {
      ...
      // Roll back: delete the newly created user so the email is free for retry
      await User.findByIdAndDelete(user._id);
      logger.error('Teacher profile creation failed, user rolled back:', profileErr.message);
      return res.status(500).json({ success: false, message: profileErr.message });
    }
...
exports.exportStudentsExcel = async (req, res) => {
  try {
    ...
    return serviceError(res, 'exportStudentsExcel error:', error);
  }
};
