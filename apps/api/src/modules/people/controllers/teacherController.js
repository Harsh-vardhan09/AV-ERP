...
exports.takeAttendance = async (req, res) => {
  // RETIRED. Attendance is now ONE record per student per day, marked by the
  // section's class teacher — see modules/attendance. This wrote one document per
  // class PERIOD with an embedded records[] array, so a student appeared once per
  // period and every percentage divided by periods rather than days.
  //
  // Left as an explicit 410 rather than deleted: a cached mobile client still
  // posting here must be told why its writes stopped, not fail silently into a
  // model nothing reads any more.
  return res.status(410).json({
    success: false,
    message:
      'Per-period attendance has been replaced by daily attendance. ' +
      'Use POST /api/v1/attendance/mark — marked once per day by the class teacher.',
    details: { code: 'ENDPOINT_RETIRED', replacedBy: '/api/v1/attendance/mark' },
  });
};
