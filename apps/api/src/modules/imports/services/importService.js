...
class ImportService {
  ...
  async previewImport(fileBuffer, fileMetadata, entityType, userContext) {
    try {
      ...
      const mapping      = ColumnMapper.mapHeaders(parsed.headers, entityConfig, { allowFuzzyMatch: true });
  ...
  async startImport(fileBuffer, fileMetadata, entityType, options = {}, userContext) {
    try {
      ...
      for (const rowObj of parsed.rows) {
        ...
        try {
          ...
          if (result.success) {
            ...
            skippedCount++;
            await this._storeError(importLog._id, userContext.schoolId, entityType, rowNumber, rawData, 'skipped', result.errors?.[0] || 'Skipped');
          } else {
            failureCount++;
            await this._storeError(importLog._id, userContext.schoolId, entityType, rowNumber, rawData, 'adapter_error', result.errors?.[0] || 'Failed');
          }
        } catch (rowErr) {
          failureCount++;
          logger.warn(`[ImportService] Row ${rowNumber} error:`, rowErr.message);
          await this._storeError(importLog._id, userContext.schoolId, entityType, rowNumber, rawData || {}, 'system_error', rowErr.message);
        }
      ...
      const finalStatus = failureCount === 0 && skippedCount === 0
        ? 'completed'
        : failureCount === 0
          ? 'completed'
          : successCount > 0
      ...
      logger.info('[ImportService] Sync import complete', {
        entity: entityType, successCount, failureCount, skippedCount, duration,
      });
  ...
  async _storeError(importLogId, schoolId, entity, rowNumber, rowData, errorType, errorMessage) {
    ...
    } catch (_) { /* best-effort — never block the import */ }
  }
  ...
  _buildInlineReferenceResolver() {
    ...
    return {
      ...
      resolveClassByName: async (name, sessionId, schoolId) => {
        ...
        if (cache[key] !== undefined) return cache[key];
        const doc = await ClassModel.findOne({ name: name?.trim(), session: sessionId, schoolId }).lean();
        cache[key] = doc;
        return doc;
      },
      resolveSectionByName: async (name, classId, sessionId, schoolId) => {
        ...
        if (cache[key] !== undefined) return cache[key];
        const doc = await SectionModel.findOne({ name: name?.trim(), classId, session: sessionId, schoolId }).lean();
        cache[key] = doc;
        return doc;
      },
    };
  ...
  async executeImport(importLogId, fileBuffer, fileMetadata, entityType, options, userContext) {
    try {
      ...
      const mapping      = ColumnMapper.mapHeaders(parsed.headers, entityConfig, { allowFuzzyMatch: true });
      const context      = {
        userId: userContext.userId, schoolId: userContext.schoolId,
        user: userContext.user, importLogId: importLog._id,
        referenceResolver: this._buildInlineReferenceResolver(),
      };

      let successCount = 0, failureCount = 0, skippedCount = 0, processedRows = 0;
      ...
      for (const rowObj of parsed.rows) {
        ...
        try {
          ...
          if (options.sessionId && !mapped.sessionId && !mapped.session) mapped.sessionId = options.sessionId;
          ...
          failureCount++;
          await this._storeError(importLog._id, userContext.schoolId, entityType, rowNumber, rawData, 'system_error', e.message);
        }
        processedRows++;
      }
      ...
      const duration    = Date.now() - startTime;
      importLog.status  = finalStatus; importLog.completedAt = new Date();
      importLog.totalRows = parsed.rows.length; importLog.processedRows = processedRows;
      importLog.successCount = successCount; importLog.failureCount = failureCount;
      importLog.skippedCount = skippedCount; importLog.duration = duration;
      await importLog.save();

      return { success: true, importLogId: importLog._id, summary: { successCount, failureCount, skippedCount } };
    } catch (error) {
  ...
  getEntityConfig(entityType)             { return this.entityConfigs[entityType]; }
