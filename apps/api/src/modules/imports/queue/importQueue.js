/**
 * ImportQueue - Bull queue for async import processing.
 * Exposes this.queue = null when Redis is unavailable; callers treat that as
...
class ImportQueue {
  constructor(redisClient) {
    ...
    // Gate mirrors queue/factory's own. It cannot be delegated: createQueue returns a
    // truthy no-op stub when Redis is off, and every caller reads a falsy this.queue as
    // "run the import synchronously" (importService.startImport). Handing them the stub
    // would take the async path against a queue that never processes, losing the import.
    if (REDIS_DISABLED || !process.env.REDIS_URL) {
      logger.warn('[ImportQueue] Redis unavailable — import will run synchronously (no queue).');
      return;
    }

    try {
      this.queue = createQueue('import');
