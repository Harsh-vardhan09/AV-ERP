...
class StudentAdapter extends BaseAdapter {
  ...
  async validateRow(row, context = {}) {
    ...
    if (!row.fatherPhone && !row.parentDetails?.father?.phone)
      errors.push('fatherPhone is required');
    ...
    if (referenceResolver && schoolId) {
      ...
      if (!sessionDoc) {
        errors.push(
          `Session "${sessionName}" not found. Please create it in Academic Sessions first.`
        );
        return errors;
      }
      ...
      const classDoc = await referenceResolver.resolveClassByName(
        className,
        sessionDoc._id,
        schoolId
      );
      if (!classDoc) {
        errors.push(
          `Class "${className}" not found in session "${sessionName}". Please ensure classes are set up.`
        );
        return errors;
      }
      ...
      const sectionDoc = await referenceResolver.resolveSectionByName(
        sectionName,
        classDoc._id,
        sessionDoc._id,
        schoolId
      ...
      if (!sectionDoc) {
        errors.push(
          `Section "${sectionName}" not found in class "${className}". Please ensure sections are set up.`
        );
      }
    }
  ...
  async create(row, context = {}) {
    const { schoolId, referenceResolver } = context;
    const { User, StudentProfile, ClassModel, SectionModel, AcademicSession, SchoolSettings } =
      getModels();
    ...
    try {
      ...
      if (normalizedEmail) {
        ...
        if (existingUser) {
          return {
            ...
            skipped: true,
            errors: [
              `Email "${normalizedEmail}" already registered. Skipped — do manual entry if needed.`,
            ],
          };
        }
      }
      ...
      if (existingProfile) {
        return {
          ...
          skipped: true,
          errors: [
            `Admission number "${finalAdmissionNo}" already exists. Skipped — do manual entry if needed.`,
          ],
        };
      }
      ...
      setImmediate(async () => {
        try {
          ...
          /* No fee structure set up yet — that's OK */
        }
      });
      ...
      logger.error('[StudentAdapter] create() error:', err.message);
  ...
  async checkDuplicate(row, context = {}) {
    ...
    if (admissionNo)
      checks.push(StudentProfile.findOne({ admissionNumber: admissionNo, schoolId }));
