 ...
 *
 * Wiring OASES means creating an OASESResult model, replacing the stub below with a
 * find on it, and mapping the docs to the MarksModel shape
...
class MarksSourceService {
  ...
  static async getMarks({
    ...
    console.log(
      `[MarksSource] Query ids=${idCandidates.map(String).join(',')} → ${marks.length} marks found`
    );
    return marks;
  }
