...
const { genereteTokenAndCookies } = require("../lib/generateTokenAndCookies");
...
exports.resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    // Hash before storing — plain token goes in email link, only hash in DB
    ...
    // TODO: send reset email with resetToken (not hashedToken) — flow is broken end to end
...
exports.changePassword = async (req, res, next) => {
  ...
  try {
    // Hash the submitted token so we can compare against the stored hash
    const hashedToken = crypto
      .createHash('sha256')
      .update(token)
      .digest('hex');
    ...
    // Clear the token fields after successful password change
...
exports.changeFirstPassword = async (req, res, next) => {
  try {
    ...
    try {
      ...
      const { sendPasswordChangedNotification } = require('../../notifications').emailService;
