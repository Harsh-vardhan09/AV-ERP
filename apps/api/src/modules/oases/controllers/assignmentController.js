...
exports.assignSingleSheet = oasesAsync(async (req, res) => {
  ...
  if (!evaluator)
    return apiError(
      res,
      'Evaluator not found in this school or does not have EVALUATOR role.',
      404
    );

  // Config is optional: used only to read dailyEvalLimit
  const config =
    (await ExamConfig.findById(sheet.examConfigId).select('dailyEvalLimit subjectCode').lean()) ||
...
exports.bulkAssign = oasesAsync(async (req, res) => {
  ...
  const config =
    (await ExamConfig.findOne({ _id: examId, schoolId: req.schoolId }).lean()) ||
...
exports.assignSheets = oasesAsync(async (req, res) => {
  ...
  const config =
    (await ExamConfig.findOne({ _id: examConfigId, schoolId: req.schoolId }).lean()) ||
...
exports.getUnassignedSheets = oasesAsync(async (req, res) => {
  const sheets = await AnswerSheet.find({
    ...
    .select('anonymousCode set processingStatus status createdAt totalPages')
    .lean();
...
exports.getEvaluators = oasesAsync(async (req, res) => {
  const evaluators = await User.find({
    schoolId: req.schoolId,
    isActive: true,
    $or: [{ oasesRole: OASES_ROLES.EVALUATOR }, { role: 'teacher' }],
  })
    .select('_id firstName lastName email role oasesRole')
    .lean();
...
exports.autoAssign = oasesAsync(async (req, res) => {
  ...
  const { Exam } = require('../../examination');
  const { SubjectMaster } = require('../../academics');
  ...
  const unassigned = await AnswerSheet.find({
    ...
    .select('_id classId sectionId subjectId')
    .lean();
  ...
  if (sampleSheet.classId && sampleSheet.subjectId) {
    ...
    const subject = await SubjectMaster.findOne({
      code: subjectCode,
      schoolId: req.schoolId,
    }).lean();
    const allClasses = await ClassModel.find({ schoolId: req.schoolId }).select('_id name').lean();
    const allSections = await SectionModel.find({ schoolId: req.schoolId })
    ...
    for (const cls of allClasses) {
      ...
      if (matchedSec) {
        matchedClassId = cls._id;
        matchedSectionId = matchedSec._id;
        break;
      }
    }
    ...
    if (!matchedClassId) {
      return apiError(
        res,
        `Could not resolve class "${classLevel}" in this school. Ensure the class name in OASES matches School Settings.`,
        404
      );
    }
  ...
  if (!teacherId) {
    return apiError(
      res,
      'No subject teacher found for this exam. Please assign a teacher in School Settings → Teacher Assignment, then retry.',
      404
    );
  }
