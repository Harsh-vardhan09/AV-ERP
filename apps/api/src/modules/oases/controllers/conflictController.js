...
const { SHEET_STATUS, EVAL_ROUNDS, NOTIFICATION_TYPES } = require('../lib/constants');
...
exports.listConflicts = oasesAsyncHandler(async (req, res) => {
  ...
  return ok(res, { sheets, total, page: Number(page), limit: Number(limit) }, 'Conflict sheets fetched.');
});
...
exports.getConflictDetail = oasesAsyncHandler(async (req, res) => {
  ...
  if (!sheet) return apiError(res, 'Conflict sheet not found.', 404);
...
exports.routeToHead = oasesAsyncHandler(async (req, res) => {
  ...
  return ok(res, null, 'Sheet routed to Head Examiner for review.');
});
...
exports.resolveConflict = oasesAsyncHandler(async (req, res) => {
  ...
  return ok(res, null, 'Conflict resolved. Sheet is now locked.');
});
