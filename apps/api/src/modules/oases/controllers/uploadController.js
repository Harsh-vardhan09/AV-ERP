...
exports.uploadSheets = oasesAsync(async (req, res) => {
  ...
  if (!config && !legacyConfig) {
    return apiError(res, 'Exam not found or does not belong to this school.', 404);
  }
  ...
  if (!files || files.length === 0) return apiError(res, 'No files uploaded.', 400);
  ...
  for (const file of files) {
    ...
    try {
      ...
      logger.error('[uploadController] Cloudinary upload failed:', uploadErr.message);
    ...
    if (!jobId) {
      setImmediate(() =>
        processAnswerSheet(sheet._id.toString(), jobMeta).catch((err) =>
          logger.error('[uploadController] Direct PDF processing failed:', err.message)
        )
      );
    }
  ...
  if (results.length > 0) {
    setImmediate(async () => {
      try {
        ...
        const { EVAL_ROUNDS, NOTIFICATION_TYPES, OASES_ROLES } = require('../lib/constants');
...
exports.reprocessSheet = oasesAsync(async (req, res) => {
  ...
  if (sheet.processingStatus !== PROCESSING_STATUS.FAILED) {
    return apiError(res, 'Only failed sheets can be reprocessed.', 400);
  }
  ...
  return ok(res, { sheetId: sheet._id }, 'Sheet queued for reprocessing.');
});
...
exports.getPageUrl = oasesAsync(async (req, res) => {
  ...
  if (sheet.processingStatus !== PROCESSING_STATUS.DONE) {
    return apiError(res, 'Sheet images not yet ready.', 409);
  }
  ...
  if (!isAdmin && !isAssigned) {
    return apiError(res, 'You are not assigned to this sheet.', 403);
  }

  const images = sheet.pageImages.length ? sheet.pageImages : sheet.s3Keys;
  if (pageIdx < 0 || pageIdx >= images.length) {
    return apiError(res, `Page ${pageNo} does not exist. Total pages: ${images.length}`, 400);
  }

  const url = await getSignedPageUrl(images[pageIdx]);
  return ok(res, { url, page: Number(pageNo), totalPages: images.length }, 'Page URL generated.');
});
