    ...
    sheets.map(async (sheet) => {
      const [e1, e2] = await Promise.all([
        EvaluationMark.findOne({ sheetId: sheet._id, round: EVAL_ROUNDS.ROUND_1, isDraft: false })
          .select('grandTotal totalMarksAwarded submittedAt')
          .lean(),
      ...
      return {
        ...
        difference:
          e1 && e2
            ? Math.abs(
                (e1.grandTotal ?? e1.totalMarksAwarded ?? 0) -
                  (e2.grandTotal ?? e2.totalMarksAwarded ?? 0)
              )
            : null,
...
exports.getConflictSheet = oasesAsync(async (req, res) => {
  const sheet = await AnswerSheet.findOne({
    ...
    .select('-rollNo -rollNoEncrypted -eval1AssignedTo -eval2AssignedTo -uploadedBy')
    .lean();
  ...
  const examConfig =
    (await ExamConfig.findById(sheet.examConfigId)
      .select('examName subjectName subjectCode totalMarks passingMarks conflictThreshold')
      .lean()) ||
    (await Exam.findById(sheet.examConfigId)
      .select('name subjectName subjectCode totalMarks passingMarks conflictThreshold')
      .lean());
...
exports.resolveConflict = oasesAsync(async (req, res) => {
  ...
  const scheme = await QuestionScheme.findOne({ examConfigId: sheet.examConfigId })
    ...
    .lean();
  const examConfig =
    (await ExamConfig.findById(sheet.examConfigId).select('totalMarks passingMarks').lean()) ||
    (await Exam.findById(sheet.examConfigId).select('totalMarks passingMarks').lean());

  // Validate marks
  const validation = validateMarks(
    { marks: marksArray, grandTotal: 0, pagesReviewed: [] },
    scheme,
    {
      ...examConfig,
      totalPages: sheet.totalPages,
  ...
  const [e1, e2] = await Promise.all([
    EvaluationMark.findOne({ sheetId, round: EVAL_ROUNDS.ROUND_1, isDraft: false })
      .select('grandTotal totalMarksAwarded')
      .lean(),
    EvaluationMark.findOne({ sheetId, round: EVAL_ROUNDS.ROUND_2, isDraft: false })
      .select('grandTotal totalMarksAwarded')
      .lean(),
  ]);
