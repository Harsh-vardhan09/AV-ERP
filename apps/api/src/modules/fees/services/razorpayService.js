...
const getRazorpay = () => {
  if (!_razorpay) {
    ...
    if (process.env.RAZORPAY_KEY_ID !== process.env.RAZORPAY_KEY_ID?.trim()) {
      console.error(
        '[razorpayService] CRITICAL: RAZORPAY_KEY_ID has whitespace — this WILL cause auth failures!'
      );
    }
    if (process.env.RAZORPAY_KEY_SECRET !== process.env.RAZORPAY_KEY_SECRET?.trim()) {
      console.error(
        '[razorpayService] CRITICAL: RAZORPAY_KEY_SECRET has whitespace — this WILL cause auth failures!'
      );
    }
...
exports.createOrder = async (amountInRupees, receiptId, notes = {}) => {
  ...
  console.log(
    '[razorpayService] createOrder → amountInPaise:',
    amountInPaise,
    '| receipt:',
    safeReceipt,
    '| notes:',
  ...
  console.log(
    '[razorpayService] Razorpay order created → id:',
    order.id,
    '| amount:',
    order.amount,
    '| status:',
...
exports.verifyWebhookSignature = (rawBody, signature) => {
  try {
    ...
    if (!secret) {
      ...
      console.error(
        '[razorpayService] RAZORPAY_WEBHOOK_SECRET not set — rejecting webhook (fail-closed).'
      );
      return false;
    }
