...
const { feePaymentReceiptTemplate } = require('../../notifications').emailTemplates;
...
exports.assignStudentFee = async (req, res) => {
  try {
    ...
    return serviceError(res, 'assignStudentFee error:', error);
  }
};
...
exports.getStudentFeeSummary = async (req, res) => {
  try {
    ...
    if (isKnownError(error.message)) return sendError(res, 400, error.message);
    return serviceError(res, 'getStudentFeeSummary error:', error);
  }
};
...
exports.getClassFeeStatus = async (req, res) => {
  try {
    ...
    if (!classId || !sessionId) return sendError(res, 400, 'classId and sessionId are required');
    ...
    const fees = await StudentFee.find({
      ...
      .select('studentId totalAssigned totalPaid totalDue status')
      .lean();
    ...
    return serviceError(res, 'getClassFeeStatus error:', error);
  }
};
...
exports.collectPayment = async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      // Scope the fee to this school — without it a crafted id reads another tenant's fee
      studentFee = await StudentFee.findOne({
        _id: installment.studentFeeId,
        schoolId: req.schoolId,
      }).session(session);
      if (!studentFee)
        throw Object.assign(new Error('Student fee record not found'), { statusCode: 404 });

      // Verify student ownership
      if (studentFee.studentId.toString() !== studentProfileId)
        throw Object.assign(new Error('Installment does not belong to this student'), {
          statusCode: 403,
        });
...
exports.backfillAllStudentFees = async (req, res) => {
  try {
    ...
    logger.error('backfillAllStudentFees error:', error);
  }
};
...
exports.updatePreviousDues = async (req, res) => {
  try {
    ...
    return serviceError(res, 'updatePreviousDues error:', error);
  }
};
