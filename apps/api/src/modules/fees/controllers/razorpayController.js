...
const razorpayService = require("../../../../src-old/services/razorpayService");
...
exports.createOrder = async (req, res) => {
    try {
        ...
        logger.debug('[createOrder] userId resolved | role: %s | studentFeeId: %s', req.user?.role, studentFeeId);
        ...
        if (req.user?.role === "student") {
            ...
            logger.debug('[createOrder] profileId resolved | studentFee.studentId: %s', studentFee.studentId);
        ...
        if (totalDue <= 0)
            return sendError(res, 400, "No outstanding dues for this student");
...
exports.verifyPayment = async (req, res) => {
    try {
        ...
        const userId = req.user?._id || req.user?.id || req.userid;
        logger.debug('[verifyPayment] userId resolved | role: %s | orderId: %s', req.user?.role, razorpay_order_id);
        ...
        if (req.user?.role === "student") {
            ...
            const profileId = await resolveProfileId(userId);
            logger.debug('[verifyPayment] profileId resolved | studentFee.studentId: %s', studentFee.studentId);
        ...
        try {
            ...
            console.error('[verifyPayment] Razorpay API fetch failed — using client amount:', apiErr.message);
        }
        ...
        notificationService.createNotification({
            ...
            metadata: { receiptNumber, paymentId: String(payment._id), gatewayPaymentId: razorpay_payment_id },
        ...
        return sendSuccess(res, 201, {
            ...
            idempotent: false,
            payment,
            summary: { totalPaid: studentFee.totalPaid, totalDue: studentFee.totalDue, status: studentFee.status },
        });
...
async function handlePaymentCaptured(paymentEntity) {
    try {
        ...
        if (!studentFeeId || !mongoose.Types.ObjectId.isValid(studentFeeId)) {
            console.warn("[webhook] payment.captured: missing or invalid studentFeeId in notes", { order_id });
            return;
        }
        ...
        console.log(`[webhook] Reconciling missed payment. orderId=${order_id}, amount=₹${amountInRupees}`);
...
async function handleRefundProcessed(refundEntity) {
    try {
        ...
        console.log(`[webhook] refund.processed: gatewayStatus updated for paymentId=${payment_id}, refundId=${refundId}`);
