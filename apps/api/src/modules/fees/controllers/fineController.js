...
const { applyFines } = require('../services/autoFineService');
...
exports.applyFines = async (req, res) => {
  try {
    ...
    return serviceError(res, '[FineController] applyFines error', { error: err.message, schoolId: req.schoolId });
  }
};
...
exports.getOverdueSummary = async (req, res) => {
  try {
    ...
    return serviceError(res, '[FineController] getOverdueSummary error', { error: err.message, schoolId: req.schoolId });
  }
};
