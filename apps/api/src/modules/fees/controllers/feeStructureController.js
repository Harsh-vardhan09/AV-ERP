...
exports.createFeeStructure = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(classId)) return sendError(res, 400, 'Invalid class ID');
    ...
    if (invalidComponent)
      return sendError(
        res,
        400,
        'Each component must have a valid feeHeadId and non-negative amount'
      );
    ...
    if (error.code === 11000)
      return sendError(
        res,
        400,
        'A fee structure for this class/stream already exists in the selected session'
      );
    ...
    return serviceError(res, 'createFeeStructure error', {
      error: error.message,
      schoolId: req.schoolId,
    });
  }
...
const backfillFeesForClass = async (classId, sessionId, className, schoolId) => {
  try {
    ...
    logger.debug(
      `[FeeBackfill] Assigning fees to ${toAssign.length} student(s) in class "${className}"...`
    );
...
exports.getFeeStructureById = async (req, res) => {
  try {
    ...
    return serviceError(res, 'getFeeStructureById error', {
      error: error.message,
      schoolId: req.schoolId,
    });
  }
...
exports.updateFeeStructure = async (req, res) => {
  try {
    ...
    return serviceError(res, 'updateFeeStructure error', {
      error: error.message,
      schoolId: req.schoolId,
    });
  }
  ...
  try {
    ...
    if (inUse)
      return sendError(
        res,
        400,
        'Cannot delete: This fee structure has student fees assigned. Reassign or remove student fees first.'
      );
    ...
    return serviceError(res, 'deleteFeeStructure error', {
      error: error.message,
      schoolId: req.schoolId,
    });
  }
