...
async function main() {
  ...
  const MarksModel = require('../src/modules/examination').MarksModel;
  ...
  console.log(
    DELETE_MODE
      ? '⚠️  DELETE MODE — records WILL be removed\n'
  ...
  const subjects = await SubjectMaster.find({ _id: { $in: subjectIds } })
    .select('name')
    .lean();
  ...
  const students = await StudentProfile.find({
    $or: [{ _id: { $in: studentIds } }, { userId: { $in: studentIds } }],
  })
    .select('firstName lastName scholarNo')
    .lean();
  const stuMap = Object.fromEntries(
    students.map((s) => [String(s._id), `${s.firstName} ${s.lastName} (${s.scholarNo})`])
  );
  ...
  if (!DELETE_MODE) {
    ...
    console.log(
      `   db.marks.deleteMany({ _id: { $in: [${toDelete.map((id) => `ObjectId("${id}")`).join(', ')}] } })`
    );
  } else {
    // Prompt confirmation
