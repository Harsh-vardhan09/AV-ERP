...
const seedMaster = async () => {
  ...
  try {
    ...
    session = doc;
    console.log(
      `✅ Step 3: Academic Session — ${created ? 'Created' : 'Already exists'}: ${session.name}`
    );
  } catch (err) {
  ...
  try {
    const spData = [
      ...
      },
      {
        ...
        rollNo: '5',
        classId: class12._id,
        sectionId: sec12A._id,
        dob: new Date('2006-07-04'),
      },
      {
        user: student6User,
        admNo: 'A-006',
  ...
  try {
    ...
    console.log(
      `✅ Step 10: Teacher-Subject Assignments — ${tsaCreated} created / ${tsaSkipped} skipped`
    );
  } catch (err) {
  ...
  try {
    ...
    console.log(
      `✅ Step 11: Class Teacher Assignments — ${ctaCreated} created / ${ctaSkipped} skipped`
    );
  } catch (err) {
  ...
  try {
    ...
    console.log(
      `   Class 10: ₹${fs10.totalAmount} | Class 11: ₹${fs11.totalAmount} | Class 12: ₹${fs12.totalAmount}`
    );
  } catch (err) {
  ...
  const makePayment = async (studentProfile, payAmount, method, note) => {
    ...
    if (!sfId) {
      console.warn(`  ⚠️  No StudentFee for ${studentProfile.firstName}, skipping payment`);
      return;
    }
  ...
  try {
    ...
    if (sf2)
      await makePayment(
        sp2,
        Math.floor(sf2.totalAssigned * 0.5),
        'online',
        'Partial payment by Priya Sharma'
      );
    ...
    if (sf5)
      await makePayment(
        sp5,
        Math.floor(sf5.totalAssigned * 0.25),
        'cash',
        'Partial payment by Vikram Singh'
      );
  ...
  try {
    ...
    billingPeriod = doc;
    console.log(
      `✅ Step 16b: Billing Period — ${created ? 'Created' : 'Already exists'}: ${billingPeriod.name}`
    );
  } catch (err) {
  ...
  try {
    ...
    const afData = [
      {
        ...
        state: 'paid',
        paidPct: 1.0,
        label: 'Full payment — Rahul Gupta',
      },
      {
        user: student2User,
        profile: sp2,
      ...
      if (item.paidPct >= 1.0) {
        ...
        for (const d of instDocs) {
          ...
          if (remaining >= d.amount) {
            ...
            d.remainingAmount = d.amount - remaining;
            d.status = 'partial';
            remaining = 0;
          }
        }
    ...
    console.log(
      `✅ Step 16c: AccountFee (Dashboard) — ${afCreated} created / ${afSkipped} skipped`
    );
    ...
    if (totals[0]) {
      ...
      console.log(
        `   Dashboard should show: Assigned=₹${t.assigned} | Collected=₹${t.paid} | Outstanding=₹${t.due} | Accounts=${t.count} | Rate=${rate}%`
      );
    }
  } catch (err) {
    ...
    const attGroups = [
      {
        ...
        sps: [sp1, sp2],
        patterns: statusPatterns10,
      },
      {
        cls: class11,
        sec: sec11A,
        teacher: teacher2User,
        sps: [sp3, sp4],
        patterns: statusPatterns11,
      },
      {
        cls: class12,
        sec: sec12A,
        teacher: teacher3User,
        sps: [sp5, sp6],
        patterns: statusPatterns12,
      },
    ];
