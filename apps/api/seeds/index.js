#!/usr/bin/env node
// Runs the seeds in dependency order, each as its own child process so one
// seed's process.exit()/disconnect() cannot kill the run
...
const SEEDS = [
  ...
  },
  {
    ...
    file: '../migrations/2026-08-02-01-migrate-global-templates.js',
    args: ['--promote'],
    enabled: true,
    required: false,
...
for (const seed of SEEDS) {
  ...
  const { status, error } = spawnSync(
    process.execPath,
    [path.join(__dirname, seed.file), ...(seed.args || [])],
    { stdio: 'inherit', cwd: API_ROOT, env: process.env }   // cwd = apps/api so dotenv finds .env
  );
