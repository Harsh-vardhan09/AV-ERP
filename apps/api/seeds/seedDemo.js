...
async function seed() {
  ...
  const subjects = await SubjectMaster.insertMany(subjectData.map((s) => ({ ...s, schoolId })));
  ...
  const teacherDefs = [
    {
      fn: 'Rajan',
      ln: 'Sharma',
      email: 'math.teacher@demoabc.edu',
      emp: 'TCH001',
      subCode: 'MAT',
      clsId: cls10Id,
      secId: sec10AId,
  ...
  teacherDefs.forEach((t) =>
    console.log(
      `   ${t.fn} ${t.ln.padEnd(10)} | ${t.email.padEnd(32)} | Teacher@123 | ${t.subSlug}`
    )
  );
  console.log('\n👨‍🎓 Students (Class 10 A — marks seeded)');
  studentDefs
    .filter((s) => s.cls._id.equals(cls10._id))
    .forEach((s) =>
      console.log(
        `   ${(s.fn + ' ' + s.ln).padEnd(16)} | ${s.scholar}@demoabc.edu | Student@123 | Roll ${s.roll}`
      )
    );
  console.log('\n📝 Exam IDs');
