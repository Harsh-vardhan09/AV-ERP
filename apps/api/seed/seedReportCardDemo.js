/**
 * seedReportCardDemo.js — end-to-end demo data for the Report Card module.
 *
 * Seeds everything the report card flow needs EXCEPT marks, on purpose:
 *
 *   student  → sees the "not yet published" state (readiness 0%)
 *   teacher  → enters marks for each subject, watching the progress bar fill
 *   student  → report card becomes viewable + downloadable once all subjects are in
 *
 * That gap is the whole point — it lets you demo the readiness gate rather than
 * landing on an already-published card.
 *
 * Run standalone:   node seed/_runner.js seedReportCardDemo.js
 * Or via the runner: npm run seed:all   (enable it in seed/index.js)
 *
 * Targets school code REPORT_DEMO_SCHOOL_CODE (default DEMO2025). The school
 * must already exist — run seedSchool.js first.
 *
