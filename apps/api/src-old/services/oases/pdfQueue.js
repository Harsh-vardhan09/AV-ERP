...
try {
  ...
  pdfQueue.on('error', (err) => {
    console.warn('[OasesPdfQueue] Queue error (non-fatal):', err.message);
  });

  console.log('✅ OasesPdfQueue initialized');
} catch (err) {
  console.warn('[OasesPdfQueue] Failed to init — Redis unavailable:', err.message);
...
const addPdfJob = async (data) => {
  try {
    ...
    console.warn('[OasesPdfQueue] addPdfJob skipped — queue unavailable:', err.message);
    return null;
  }
};
