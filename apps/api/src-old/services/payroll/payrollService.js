 ...
 * ─────────────────────────────────────
 * FIXES APPLIED:
 *  - Bug 1: taxConfigService.getEffectiveConfig now exists
 *  - Bug 2: Payslip.create() field mapping corrected to match schema
 *  - Bug 3: payrollId is now accepted and passed through to Payslip.create()
 *  - Bug 7: academicYearId validated before Payroll.create()
 *  - Issue 10: All console.log replaced with logger
 */
 ...
 * ─────────────────────────────────────────────────────────────────
 * FIXES APPLIED:
 *  - Bug 2: Payslip.create() now uses correct schema field names
 *  - Bug 3: payrollId is accepted and passed to Payslip (optional for ad-hoc)
 *  - SalaryStructure now populated with component names for proper earnings snapshot
 *  - Components resolved via resolveComponentAmount before calculatePayroll
 */
