...
exports.getalluploadassignment = async (req, res, next) => {
  ...
  exports.getassignmentbyidupload = async (req, res, next) => {
    try {
      ...
      const { assignmentid ,studentid } = req.params;
      const totalassignmentcomplete = await Assignmentupload.countDocuments({ studentid, schoolId: req.schoolId })
        // SECURITY: scope to current school
        const uploadassignments = await Assignmentupload.find({ assignmentid, studentid, schoolId: req.schoolId });
      if (uploadassignments.length === 0) {
        return res.status(404).json({
          success: false,
          message: 'No assignments uploaded for this assignment',
  ...
  exports.getassignmentstudentscount = async (req, res, next) => {
    try {
      ...
      // SECURITY: scope to current school
      const totalassignmentcomplete = await Assignmentupload.countDocuments({ studentid, semester, schoolId: req.schoolId })
      res.status(200).json({
        success: true,
        totalassignmentcomplete
      });
