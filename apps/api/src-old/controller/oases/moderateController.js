...
const { getSignedPageUrl }  = require('../../services/oases/pdfService');
const { validateMarks }     = require('../../services/oases/marksValidation.service');
...
exports.listConflicts = oasesAsync(async (req, res) => {
  ...
  return oasesSuccess(res, { sheets: enriched, total, page: Number(page) }, 'Conflict sheets fetched.');
});
...
exports.getConflictSheet = oasesAsync(async (req, res) => {
  ...
  if (!sheet) return oasesError(res, 'Conflict sheet not found or already resolved.', 404);
...
exports.resolveConflict = oasesAsync(async (req, res) => {
  ...
  if (!remarks || remarks.trim().length < 20) {
    return oasesError(res, 'Remarks are required and must be at least 20 characters.', 400);
  }
  ...
  if (!validation.isValid) {
    return oasesError(res, 'Validation failed.', 400, {
  ...
  return oasesSuccess(res, { grandTotal, sectionTotals }, 'Conflict resolved. Sheet is now locked.');
});
