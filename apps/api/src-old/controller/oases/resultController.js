...
exports.generateResults = oasesAsyncHandler(async (req, res) => {
  ...
  if (lockedSheets.length === 0) {
    return oasesError(res, 'No locked sheets found for this exam config.', 400);
  }
  ...
  return oasesSuccess(res, { generated: results.length }, `${results.length} result(s) generated.`);
});
...
exports.listResults = oasesAsyncHandler(async (req, res) => {
  ...
  return oasesSuccess(res, { results, total, page: Number(page), limit: Number(limit) }, 'Results fetched.');
});
...
exports.publishResults = oasesAsyncHandler(async (req, res) => {
  ...
  return oasesSuccess(res, { published: updated.modifiedCount }, 'Results published.');
});
