...
// const getSavedCoScholasticTemplateId = async ({ studentId, schoolId, sessionId }) => {
//   const reportCard = await ReportCard.findOne({ studentId, schoolId, session: sessionId }).select('_id').lean();
//   if (!reportCard) return null;
//   const mark = await CoScholasticMark.findOne({
//     reportCardId: reportCard._id,
//     schoolId,
//     templateId: { $ne: null },
//   }).sort({ updatedAt: -1 }).select('templateId').lean();
//   return mark?.templateId || null;
// };
...
exports.generateReport = async (req, res) => {
  ...
  try {
    ...
    // const savedCoTemplateId = templateId ? null : await getSavedCoScholasticTemplateId({
    //   studentId,
    //   schoolId,
    //   sessionId: session._id,
    // });
    // templateId is read-only (const from req.body destructure). Use a local alias.
    const resolvedTemplateId = templateId || null; // only use explicit templateId if provided
    const template = await resolveTemplateForStudent({
      studentId,
      schoolId,
      examType,
      // templateId: templateId || savedCoTemplateId || null,
    });
    ...
    // FIX 2: Generate PDF — TemplateParserService.render() is called ONCE internally.
    // The previous explicit render() call above was a duplicate and has been removed.
    ...
    logger.error('[DynamicReport] Generate error:', error);
    return res.status(500).json({
      success: false,
...
exports.generateBulkReports = async (req, res) => {
  try {
    ...
    // Pass templateId as override only if caller explicitly requested one template.
    const bulkBaseTemplateId = templateId || null;
    ...
    for (const studentId of studentIds) {
      try {
        // const savedCoTemplateId = bulkBaseTemplateId ? null : await getSavedCoScholasticTemplateId({
        //   studentId,
        //   schoolId,
        //   sessionId: session._id,
        // });

        // Resolve template for this specific student's class
        const template = await resolveTemplateForStudent({
          studentId,
          schoolId,
          examType,
          // templateId: bulkBaseTemplateId || savedCoTemplateId || null,
    ...
    logger.error('[DynamicReport] Bulk generate error:', error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.previewReport = async (req, res) => {
  try {
    ...
    logger.error('[DynamicReport] Preview error:', error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.downloadReport = async (req, res) => {
  try {
    ...
    logger.error('[DynamicReport] Download error:', error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.getReports = async (req, res) => {
  try {
    ...
    logger.error('[DynamicReport] Get reports error:', error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.deleteReport = async (req, res) => {
  try {
    ...
    logger.error('[DynamicReport] Delete error:', error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.getStats = async (req, res) => {
  try {
    ...
    logger.error('[DynamicReport] Stats error:', error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.validateTemplate = async (req, res) => {
  try {
    ...
    console.log(`[Validate] Template "${template.name}" | Fields: ${totalFields} | Resolved: ${resolvedCount} | Missing: ${renderResult.missingFields.length}`);
