...
const run = async () => {
  ...
  result.admin = await User.findOne({ email: 'admin@school.com' })
    .select('email schoolId role')
    .lean();
  result.teacher = await User.findOne({ email: 'rc_teacher@school.com' })
  ...
  result.sampleStudents = await StudentProfile.find({})
    .limit(3)
    .select('firstName lastName schoolId session classId rollNo status')
    .lean();
