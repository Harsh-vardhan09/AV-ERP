/**
 * Migrate Report Card tenancy fields (schoolId) for existing data.
 *
 * What it fixes:
 *  - Exam documents missing schoolId (derives from createdBy user's schoolId)
 *  - ExamSubjectConfig documents missing schoolId (derives from exam.schoolId)
 *
 * Usage (PowerShell):
 *  cd backend
 *  node scripts/migrate-reportcard-tenancy.js
 *
 * Requirements:
 *  - MONGODB_URI env must be set (same as backend uses)
 */
