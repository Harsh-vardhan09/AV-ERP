/**
 * cleanStaleMarks.js — Diagnostic + Cleanup for Stale Dynamic Marks
 *
 * Run: node scripts/cleanStaleMarks.js          → dry-run (shows what will be deleted)
 *      node scripts/cleanStaleMarks.js --delete  → actually deletes the bad records
 *
 * What it does:
 *  1. Finds all MarksModel records grouped by subjectId for each student
 *  2. Prints them so you can see exactly what is in the DB
 *  3. With --delete: removes every dynamic marks record for subjects
 *     whose total obtained marks look incorrect (all-subjects bleed artifact)
 */
...
async function main() {
  ...
  const MarksModel    = require('../src/models/MarksModel');
